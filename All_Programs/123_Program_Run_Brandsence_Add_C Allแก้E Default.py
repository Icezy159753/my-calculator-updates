...
class SpssProcessorApp(QMainWindow):
    ...
    def _run_full_transformation_logic(self):
        try:
            ...
            def _col_sort_key(x):
                ...
                return (prefix, int(suffix[0]), int(suffix))
    ...
    def _create_summary_df_logic(self, primary_filter, cross_filter):
        ...
        try:
            ...
            if 'A' in corr_df.columns:
                e_cols_for_corr = sorted([c for c in corr_df.columns if re.match(r'^E_\d+$', c)], key=lambda x: int(x.split('_')[1][0]))
    ...
    def _prepare_final_excel_df(self, final_summary_df):
        ...
        e_cols = sorted([c for c in final_summary_df.columns if c.startswith('E_') and 'cor' not in c], key=lambda x: (int(x.split('_')[1][0]), int(x.split('_')[1])))
        ...
        core_cols = sorted([c for c in final_summary_df.columns if c.startswith('CorE_')], key=lambda x: (int(x.split('_')[1][0]), int(x.split('_')[1])))
