...
class SpssProcessorApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self._qc_removed_long_df = pd.DataFrame()
    ...
    def _apply_qc_candidate_rows(self, selected_rows):
        ...
        factor_metrics = [
            c for c in ('N_S', 'N_P', 'N_C', 'N_E')
            if c in self.transformed_df.columns]
        factor_values = {metric: [] for metric in factor_metrics}
        for _, row in selected.iterrows():
            ...
            if id_col not in self.transformed_df.columns \
                ...
                for metric in factor_metrics:
                    factor_values[metric].append(np.nan)
                continue
            row_mask = (
            ...
            for metric in factor_metrics:
                matched = self.transformed_df.loc[row_mask, metric]
                factor_values[metric].append(
                    float(matched.mean()) if not matched.empty else np.nan)
            remove_mask |= row_mask

        selected['Rows_Removed'] = removed_by_candidate
        for metric in factor_metrics:
            selected[metric] = factor_values[metric]
        ...
        # เก็บแถวที่ถูกตัดไว้ให้ตาราง Cross ของหน้ารีวิว QC ใช้ระบายสี
        self._qc_removed_long_df = self.transformed_df.loc[
            remove_mask].copy()
    ...
    def _qc_cut_summary_stats(self):
        """สรุปจำนวนแถว Long Format ก่อน/หลังตัด Hard QC/Review"""
        full_df = self._qc_full_transformed_df
        if full_df is not None:
            ...
            total = 0
        kept = len(self.transformed_df) \
            if self.transformed_df is not None else 0
        cut = max(total - kept, 0)
        cut_pct = (cut / total * 100) if total else 0.0
        excluded = self.qc_excluded_df
        has_excluded = excluded is not None and not excluded.empty
        hard_n = int((excluded['QC_Level'] == 'Hard QC').sum()) \
            if has_excluded else 0
        review_n = int((excluded['QC_Level'] == 'Review').sum()) \
            if has_excluded else 0
        return {
            'total': total, 'kept': kept, 'cut': cut,
            'cut_pct': cut_pct, 'hard_n': hard_n, 'review_n': review_n,
        }

    def _build_qc_cut_crosstab(self, metric):
        """ตาราง Cross จาก Long Format เต็มก่อนตัด Hard QC/Review

        แถว = ค่า metric (ปัด 2 ตำแหน่ง), คอลัมน์ = A
        คืน (total, cut, a_values, x_values) หรือ None ถ้าทำไม่ได้
        """
        full = self._qc_full_transformed_df
        if full is None or full.empty:
            return None
        if 'A' not in full.columns or metric not in full.columns:
            return None

        a = pd.to_numeric(full['A'], errors='coerce')
        x = pd.to_numeric(full[metric], errors='coerce')
        sub = pd.DataFrame({'A': a, 'X': x.round(2)}).dropna()
        if sub.empty:
            return None
        total = pd.crosstab(sub['X'], sub['A'])

        cut = None
        removed = self._qc_removed_long_df
        if removed is not None and not removed.empty \
                and 'A' in removed.columns and metric in removed.columns:
            ra = pd.to_numeric(removed['A'], errors='coerce')
            rx = pd.to_numeric(removed[metric], errors='coerce')
            rsub = pd.DataFrame({'A': ra, 'X': rx.round(2)}).dropna()
            if not rsub.empty:
                cut = pd.crosstab(rsub['X'], rsub['A'])
        if cut is None:
            cut = total * 0
        cut = cut.reindex(
            index=total.index, columns=total.columns).fillna(0)
        return (total, cut, list(total.columns), list(total.index))

    def _build_qc_cut_crosstab_table(self, metric):
        """สร้าง QTableWidget ของตาราง Cross A x metric สำหรับหน้ารีวิว QC

        สีเดียวกับตาราง Cross ของ Compute C:
        🟩 เขียว = แถวที่ใช้วิเคราะห์  🟨 เหลือง = ถูกตัดออกทั้งช่อง
        🟧 ส้ม = ถูกตัดบางส่วนในช่องนั้น
        """
        built = self._build_qc_cut_crosstab(metric)
        if built is None:
            lb = QLabel(f"ไม่มีข้อมูลสำหรับ {metric}")
            lb.setStyleSheet("color:#888; padding:20px;")
            return lb

        total, cut, a_vals, x_vals = built
        n_rows, n_cols = len(x_vals), len(a_vals)
        ...
        for r, xv in enumerate(x_vals):
            for c, av in enumerate(a_vals):
                ...
                elif cnt and n_cut >= cnt:
                    item.setBackground(yellow)
                    item.setToolTip(f"ถูกตัดออกทั้งหมด {cnt} แถว")
                elif cnt:
                    item.setBackground(mixed)
                    item.setToolTip(f"ตัด {n_cut} / {cnt} แถว")
                tw.setItem(r, c, item)
    ...
    def show_qc_cut_summary_panel(self, on_continue, on_cancel):
        """หน้าสรุปหลังตัด Hard QC/Review — เช็คก่อนเริ่มวิเคราะห์จริง"""
        self._clear_right_panel()
        ...
        head = QLabel("✅  ตรวจ QC เสร็จแล้ว — ก่อนเริ่มวิเคราะห์")
        head.setStyleSheet(
            "color:#2E7D32; font-size:21px; font-weight:700;"
            "background:transparent;")
        pl.addWidget(head)
        ...
        if src:
            fname = QLabel(os.path.basename(src))
            fname.setStyleSheet(
                "color:#78909C; font-size:11px;"
                "background:transparent;")
            fname.setWordWrap(True)
        ...
        stats = self._qc_cut_summary_stats()

        sec = QLabel("ตัดชุดด้วยเงื่อนไข Hard QC / Review")
        sec.setStyleSheet(
            "color:#B71C1C; font-size:13px; font-weight:700;"
            "background:transparent; padding-top:6px;")
        pl.addWidget(sec)
        ...
        tiles.setSpacing(10)
        tiles.addWidget(self._stat_tile(
            f"{stats['total']:,}", "ข้อมูลทั้งหมด", "แถว Long Format",
            "#37474F", "#ECEFF1"))
        tiles.addWidget(self._stat_tile(
            f"{stats['kept']:,}", "ใช้วิเคราะห์",
            f"เหลือ  ({100 - stats['cut_pct']:.1f}%)",
            "#2E7D32", "#E8F5E9"))
        tiles.addWidget(self._stat_tile(
            f"{stats['cut']:,}", "ตัดออก (Hard QC/Review)",
            f"Hard QC {stats['hard_n']:,} | Review {stats['review_n']:,}"
            f"  ({stats['cut_pct']:.1f}%)",
            "#C62828", "#FFEBEE"))
        pl.addLayout(tiles)
        ...
        drop.setStyleSheet(
            ...
            "border-top-right-radius:5px;"
            "border-bottom-right-radius:5px;")
        bl.addWidget(keep, max(int(round(
            (100 - stats['cut_pct']) * 10)), 1))
        bl.addWidget(drop, max(int(round(stats['cut_pct'] * 10)), 1))
        pl.addWidget(bar)

        # ---- ปุ่มดำเนินการ (อยู่บนสุด ไม่ต้องเลื่อนลงไปหา) ----
        nxt = QLabel(
            "ตรวจสอบตาราง Cross และรายการที่ตัดออกด้านล่างให้เรียบร้อย "
            "ก่อนกด 'ดำเนินการวิเคราะห์ต่อ' — หากไม่ถูกต้อง กด 'ยกเลิก' "
            "แล้วกดปุ่ม 'วิเคราะห์และส่งออก Excel' ใหม่เพื่อเลือก QC อีกครั้ง")
        nxt.setWordWrap(True)
        nxt.setStyleSheet(
            "color:#0D47A1; font-size:12px; font-weight:600;"
            "background:#E3F2FD; border-left:4px solid #1976D2;"
        ...
        btn_cancel = QPushButton("ยกเลิก")
        btn_cancel.setStyleSheet(_BTN_STYLES['outline'])
        btn_continue = QPushButton("✓  ดำเนินการวิเคราะห์ต่อ")
        btn_continue.setStyleSheet(_BTN_STYLES['success'])
        for btn in (btn_cancel, btn_continue):
            btn.setMinimumHeight(42)
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
        actions.addWidget(btn_cancel, 1)
        actions.addWidget(btn_continue, 2)
        ...
        btn_continue.clicked.connect(_do_continue)
        btn_continue.setDefault(True)

        # ---- ตาราง Cross A x N_S/N_P/N_C/N_E (สไตล์เดียวกับ Compute C) ----
        if self._qc_full_transformed_df is not None:
            sec_cross = QLabel("ตาราง Cross : A × N_S / N_P / N_C / N_E")
            sec_cross.setStyleSheet(
                "color:#B71C1C; font-size:13px; font-weight:700;"
                "background:transparent; padding-top:10px;")
            pl.addWidget(sec_cross)

            cross_hint = QLabel(
                "นับจาก Long Format เต็มก่อนตัด — แถว = ค่าตัวชี้วัด, "
                "คอลัมน์ = A\n"
                "🟩 แถวที่ใช้วิเคราะห์   "
                "🟨 ถูกตัด Hard QC/Review ทั้งช่อง   "
                "🟧 ถูกตัดบางส่วน (ชี้เมาส์ดูจำนวน)")
            cross_hint.setWordWrap(True)
            cross_hint.setStyleSheet(
                "color:#78909C; font-size:10px;"
                "background:transparent;")
            pl.addWidget(cross_hint)

            cross_tabs = QTabWidget()
            for metric in ('N_S', 'N_P', 'N_C', 'N_E'):
                page_w = self._build_qc_cut_crosstab_table(metric)
                cross_tabs.addTab(page_w, f"  {metric}  ")

            # ปรับความสูงตามแท็บที่กำลังแสดง — แท็บที่แถวน้อย
            # จะไม่เหลือช่องว่างค้างก่อนถึงตารางถัดไป
            def _sync_cross_height(idx):
                w = cross_tabs.widget(idx)
                if w is not None:
                    content_h = max(
                        w.minimumHeight(), w.sizeHint().height())
                    cross_tabs.setFixedHeight(
                        content_h
                        + cross_tabs.tabBar().sizeHint().height() + 16)

            cross_tabs.currentChanged.connect(_sync_cross_height)
            _sync_cross_height(cross_tabs.currentIndex())
            pl.addWidget(cross_tabs)

        excluded = self.qc_excluded_df
        if excluded is not None and not excluded.empty:
            sec2 = QLabel(
                f"รายการที่ตัดออก ({len(excluded):,} คู่ SBJNUM+Index1)")
            sec2.setStyleSheet(
                "color:#B71C1C; font-size:13px; font-weight:700;"
                "background:transparent; padding-top:10px;")
            pl.addWidget(sec2)
            ...
            legend = QLabel(
                "🟥 QC_Level: Hard QC ตัดเสมอ   🟨 QC_Level: Review\n"
                "ไฮไลท์สีในคอลัมน์ A/Attribute/P = ค่าที่ทำให้แถวนี้โดนตัด — "
                "🟧 ส้ม เข้าเงื่อนไข Hard QC   🟨 เหลือง เข้าเงื่อนไข Review")
            ...
            # สีเดียวกับตาราง Cross A x N_S/N_P/N_C/N_E ของ Compute C:
            # เขียว = ปกติ, เหลือง = โดนเงื่อนไข Review ตัด,
            # ส้ม = โดนเงื่อนไข Hard QC ตัด (รุนแรงกว่า)
            hard_hl = QColor('#F4B183')
            review_hl = QColor('#FFE699')

            def _reason_level(reason, hard_keys, review_keys):
                reason = reason or ''
                if any(k in reason for k in hard_keys):
                    return hard_hl
                if any(k in reason for k in review_keys):
                    return review_hl
                return None

            a_hard_keys = ['Q1=1 แต่เลือก Attribute', 'Q1=6–8']
            a_review_keys = [
                'Q1≤3 แต่เลือก P', 'Q1=3 แต่เลือก Attribute เกือบ']
            attr_hard_keys = ['Q1=1 แต่เลือก Attribute', 'Q1=6–8']
            attr_review_keys = ['Q1=3 แต่เลือก Attribute เกือบ']
            p_hard_keys = []
            p_review_keys = ['Q1≤3 แต่เลือก P']

            float_cols = {'N_S', 'N_P', 'N_C', 'N_E'}
            has_attr = {'Attribute_Count', 'Attribute_Total'} <= set(
                excluded.columns)
            has_p = {'P_Count', 'P_Total'} <= set(excluded.columns)
            cols = (
                ['QC_Level', 'SBJNUM', 'Index1', 'Index1_Label',
                 'A_Original']
                + (['Attribute'] if has_attr else [])
                + (['P'] if has_p else [])
                + ['N_S', 'N_P', 'N_C', 'N_E', 'QC_Reason', 'Rows_Removed'])
            cols = [
                c for c in cols
                if c in excluded.columns or c in ('Attribute', 'P')]
            table = QTableWidget(len(excluded), len(cols))
            ...
            table.setAlternatingRowColors(True)
            table.verticalHeader().setVisible(False)
            for row_idx, (_, row) in enumerate(excluded.iterrows()):
                reason = row.get('QC_Reason', '')
                for col_idx, col in enumerate(cols):
                    highlight = None
                    if col == 'Attribute':
                        text = (
                            f"{int(row['Attribute_Count'])}/"
                            f"{int(row['Attribute_Total'])}")
                        highlight = _reason_level(
                            reason, attr_hard_keys, attr_review_keys)
                    elif col == 'P':
                        text = (
                            f"{int(row['P_Count'])}/"
                            f"{int(row['P_Total'])}")
                        highlight = _reason_level(
                            reason, p_hard_keys, p_review_keys)
                    else:
                        value = row[col]
                        ...
                        if col == 'A_Original':
                            highlight = _reason_level(
                                reason, a_hard_keys, a_review_keys)
                    item = QTableWidgetItem(text)
                    if col in float_cols or col in ('Attribute', 'P'):
                        item.setTextAlignment(
                            Qt.AlignmentFlag.AlignRight
                            | Qt.AlignmentFlag.AlignVCenter)
                    if col == 'QC_Level':
                        item.setBackground(QColor(
                            '#FFEBEE' if row[col] == 'Hard QC'
                            else '#FFF8E1'))
            ...
            header = table.horizontalHeader()
            stretch_col = (
                cols.index('QC_Reason') if 'QC_Reason' in cols
                else len(cols) - 1)
            for col_idx in range(len(cols)):
                header.setSectionResizeMode(
                    col_idx,
                    QHeaderView.ResizeMode.Stretch
                    if col_idx == stretch_col
    ...
    def run_analysis_and_export(self, automated=False):
        ...
        if self._settings_batch_active:
            # คิวอัตโนมัติไม่มีคนคอยกดยืนยัน จึงรันต่อทันที
            _continue_after_qc_review()
            return

        def _cancel_after_qc_review():
            self.update_status("ยกเลิกก่อนเริ่มวิเคราะห์", "warning")

        self.show_qc_cut_summary_panel(
            on_continue=_continue_after_qc_review,
            on_cancel=_cancel_after_qc_review)
