...
class App:
    ...
    def __init__(self, root):
        ...
        self.raw_df_spss = None
        self.spss_path = tk.StringVar()
        self.selected_encoding = tk.StringVar(value='Auto-Detect (None)')
        ...
        self.notebook.add(self.tab1, text="  ค้นหา 'อื่นๆ' (มี Freq)  ")
        self.notebook.add(self.tab3, text="  'อื่นๆ' ทั้งหมด (All Others)  ")
        self.notebook.add(self.tab2, text="  ตรวจสอบ Raw Data (Excel)  ")
    ...
    def configure_styles(self):
        ...
        # <<< เพิ่ม Style ใหม่สำหรับปุ่มสีส้มตรงนี้ >>>
        ORANGE_COLOR = "#F39C12" # สีส้ม
        ORANGE_DARK_COLOR = "#D35400" # สีส้มเข้ม
        style.configure('Orange.TButton',
                        font=self.DEFAULT_FONT, # ใช้ฟอนต์ธรรมดาเหมือนปุ่มเดิม
                        background=ORANGE_COLOR,
                        foreground=self.BUTTON_TEXT_COLOR,
                        borderwidth=0,
                        padding=(10, 2)) # เพิ่ม padding เล็กน้อยให้ปุ่มดูสวยขึ้น
        style.map('Orange.TButton',
                  background=[('active', ORANGE_DARK_COLOR), ('pressed', ORANGE_DARK_COLOR)],
    ...
    def create_spss_finder_tab(self):
        ...
        # --- MODIFIED LINE ---
        browse_spss_button = ttk.Button(file_frame, text="เลือกไฟล์...", command=lambda: self.browse_file(self.spss_path, "SPSS"), style='Orange.TButton'); browse_spss_button.grid(row=0, column=2, sticky="e", padx=5, pady=5)
    ...
    def create_data_checker_tab(self):
        ...
        file_frame.pack(fill=tk.X, pady=5, ipady=5)
        settings_label = ttk.Label(file_frame, text="ไฟล์ตั้งค่า (.xlsx):", background=self.FRAME_COLOR); settings_label.grid(row=0, column=0, sticky="w", padx=5, pady=5); settings_entry = ttk.Entry(file_frame, textvariable=self.settings_path, width=70, font=self.DEFAULT_FONT); settings_entry.grid(row=0, column=1, sticky="ew", padx=5, pady=5); 
        # --- MODIFIED LINE ---
        browse_settings_button = ttk.Button(file_frame, text="เลือกไฟล์...", command=lambda: self.browse_file(self.settings_path, "Excel"), style='Orange.TButton'); browse_settings_button.grid(row=0, column=2, sticky="e", padx=5, pady=5)
        raw_data_label = ttk.Label(file_frame, text="ไฟล์ Raw Data (.xlsx):", background=self.FRAME_COLOR); raw_data_label.grid(row=1, column=0, sticky="w", padx=5, pady=5); raw_data_entry = ttk.Entry(file_frame, textvariable=self.raw_data_path, width=70, font=self.DEFAULT_FONT); raw_data_entry.grid(row=1, column=1, sticky="ew", padx=5, pady=5); 
        # --- MODIFIED LINE ---
        browse_raw_data_button = ttk.Button(file_frame, text="เลือกไฟล์...", command=lambda: self.browse_file(self.raw_data_path, "Excel"), style='Orange.TButton'); browse_raw_data_button.grid(row=1, column=2, sticky="e", padx=5, pady=5)
        file_frame.columnconfigure(1, weight=1)
        
        button_frame_tab2 = ttk.Frame(self.tab2)
        button_frame_tab2.pack(pady=10)

        check_button = ttk.Button(button_frame_tab2, text="เริ่มการตรวจสอบ", command=self.start_checking, style="Accent.TButton")
        check_button.pack(side=tk.LEFT, ipady=5, padx=(0, 5))

        export_check_button = ttk.Button(button_frame_tab2, text="Export Excel (เฉพาะที่ Freq > 0)", command=self.export_check_results_to_excel, style="Success.TButton")
        export_check_button.pack(side=tk.LEFT, ipady=5, padx=(5, 0))
    ...
    def export_check_results_to_excel(self):
        if not self.check_tree.get_children():
            messagebox.showwarning("ไม่มีข้อมูล", "กรุณาทำการตรวจสอบข้อมูลก่อนทำการบันทึก")
            return

        summary_data = []
        for item_id in self.check_tree.get_children():
            values = self.check_tree.item(item_id, 'values')
            if int(values[3]) > 0:
                summary_data.append({
                    'variable': values[0],
                    'code': int(values[1]),
                    'label': values[2],
                    'new_frequency': int(values[3])
                })

        if not summary_data:
            messagebox.showinfo("ไม่มีข้อมูล", "ไม่พบรายการที่มีความถี่ (New Freq) มากกว่า 0 ให้บันทึก")
            return

        raw_data_file = self.raw_data_path.get()
        if not raw_data_file:
            messagebox.showerror("ไม่พบไฟล์", "ไม่พบเส้นทางไปยังไฟล์ Raw Data (.xlsx)\nกรุณาเลือกไฟล์ในขั้นตอนที่ 1 ก่อน")
            return

        filepath = filedialog.asksaveasfilename(
            defaultextension=".xlsx",
            filetypes=[("Excel Workbook", "*.xlsx")],
            title="บันทึกผลการตรวจสอบเป็น Excel"
        )
        if not filepath:
            return
        ...
        try:
            ...
            all_raw_columns = list(df_raw.columns)

            df_summary = pd.DataFrame(summary_data)
            df_summary['Go to'] = ''
            df_summary = df_summary[['variable', 'code', 'label', 'new_frequency', 'Go to']]

            unique_groups = df_summary.drop_duplicates(subset=['variable'])
            grouped_summary = df_summary.groupby('variable')
            ...
            for _, summary_row in unique_groups.iterrows():
                group_name = summary_row['variable']
                group_df = grouped_summary.get_group(group_name)
                target_codes = group_df['code'].tolist()
                ...
                for row_idx in rows_to_link:
                    summary_sheet.write_url(
                        row_idx + 1, df_summary.columns.get_loc('Go to'),
                        f"internal:'{sheet_name}'!A1", link_format, f"Go to {sheet_name}"
                    )
                
                if group_name.endswith(('_O', '$')):
                    vars_in_group = natsorted([c for c in all_raw_columns if str(c).startswith(group_name)])
                else:
                    vars_in_group = [group_name]

                if not vars_in_group:
                    print(f"Warning: Could not find any columns for group '{group_name}' in the raw data file.")
                    continue
                
                extra_oth_cols = []
                try:
                    last_var_index = all_raw_columns.index(vars_in_group[-1])
                    extra_oth_cols = self._find_next_oth_block(all_raw_columns, last_var_index)
                ...
                for var in vars_in_group:
                    if var in df_raw.columns:
                        mask |= df_raw[var].isin(target_codes)
                
                detail_df = df_raw.loc[mask, cols_to_show].copy()
                detail_df.to_excel(writer, sheet_name=sheet_name, index=False)
                detail_sheet = writer.sheets[sheet_name]
                
                detail_sheet.write_url('A1', f"internal:'Summary'!A1", link_format, 'Back to Summary')
                ...
                for i, code_to_highlight in enumerate(target_codes):
                    ...
                    detail_sheet.conditional_format(
                        1, 1, max_row, max_col,
                        {'type': 'cell', 'criteria': '==', 'value': float(code_to_highlight), 'format': cell_format}
                    )
            
            messagebox.showinfo("สำเร็จ", f"บันทึกผลลัพธ์ลงในไฟล์:\n{filepath}")
