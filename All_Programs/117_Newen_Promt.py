...
class App:
    ...
    def load_ai_result_excel(self):
        ...
        try:
            ...
            def _normalize(df: pd.DataFrame) -> pd.DataFrame:
                # ชื่อคอลัมน์และค่า: ตัดช่องว่างหัว-ท้ายทั้งหมด
            ...
            if 'Var' in xls.sheet_names:
                ...
                messagebox.showinfo("โหลดสำเร็จ",
                                    f"โหลดข้อมูลจากชีท 'Var' เรียบร้อย\nไฟล์: {os.path.basename(filepath)}")
            ...
            if 'Value' in xls.sheet_names:
                ...
                # จัดชื่อคอลัมน์ Label_EN ให้มาตรฐานเดียวกัน
    ...
    def save_labels_to_spss(self):
        ...
        import io, pandas as pd, tkinter as tk, re

        update_var_df = None
        update_value_df = None

        # หา column แบบไม่สนใจตัวพิมพ์เล็ก/ใหญ่
        ...
        # แปลงชนิดของ value ให้เข้ากับชนิด key ที่มีอยู่เดิมใน labels ของตัวแปรนั้น
        def _coerce_value_type(val, var, value_labels, meta):
            # ถ้ามี label เดิม ให้ยึดชนิดของ key เดิมเป็นหลัก
            ...
            # ถ้าไม่มี label เดิม ดูชนิดจาก meta (0 = numeric, >0 = string width)
            try:
        ...
        # เลือกชุดข้อมูลจาก UI หรือจากกล่องข้อความ
        ...
        if update_var_df is None and update_value_df is None and ai_output_text:
            # รองรับกรณีวางผลลัพธ์เป็น TSV ในกล่องข้อความ
            try:
        ...
        delimiter = self.ma_var.get()  # เช่น "_O" หรือ "" (ว่าง)
        meta = self.spss_meta

        # เตรียม DataFrame ที่จะเขียนกลับ (คงลำดับคอลัมน์เดิมทั้งหมด)
        try:
        ...
        # สำเนา label เดิมออกมาก่อน (ป้องกันแก้ meta ต้นฉบับ)
        ...
        if update_value_df is not None:
            ...
            # ไม่บังคับ numeric ทั้งคอลัมน์ – แปลงทีละบรรทัดให้ตรงชนิด
            for _, r in update_value_df.iterrows():
                ...
                # แปลงชนิดของ Value ให้เข้ากับชนิด key เดิม (หรือชนิดตัวแปร)
                try:
                    vv = _coerce_value_type(val, var, new_value_labels, meta)
                except Exception:
                    continue

                # นโยบาย "ลบ" label: ว่างจริง หรือกำหนดพิเศษ <<DEL>> / <<DELETE>>
        ...
        # เตรียม metadata สำหรับเขียนกลับ
