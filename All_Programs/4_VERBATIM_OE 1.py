...
def load_items_from_settings_file(path: str) -> tuple[list[dict], list[str]]:
...
def auto_fit_columns(ws, min_width: int = 8, max_width: int = 60):
    for cc in range(1, ws.max_column + 1):
        max_len = 0
...
def run_merge(
    ...
    for cfg in var_configs:
        ...
        if var_name == "":
            continue
        merged, warnings = load_verbatim_parts(text_data, var_name)
        ...
        if ns_base not in code_groups:
            code_groups[ns_base] = {"header_row": header_row, "rows": {}}
        ...
        details.append(
            {
                "var": var_name,
                "status": "SUCCESS",
                "note": "Completed" if matched_rows > 0 else "No matched SbjNum",
    ...
    for ns_base in group_order:
        ...
        row_slots_map: dict[int, list[tuple[str, str]]] = grp["rows"]
        max_codes = max((len(v) for v in row_slots_map.values()), default=0)
        if max_codes <= 0:
            continue
        ...
        created_code_groups.append({"base": ns_base, "new_cols": max_codes})
    ...
    if "New Translators" in wb.sheetnames:
        del wb["New Translators"]
    ws_new = wb.create_sheet("New Translators")
    ws_new.append(["No.", "Translator", "New Code Cols"])
    row_no = 1
    for g in created_code_groups:
        base = txt(g.get("base"))
        new_cols = int(g.get("new_cols", 0))
        if base == "" or new_cols <= 0:
            continue
        ws_new.append([row_no, base, new_cols])
        row_no += 1

    auto_fit_columns(ws_sum)
    auto_fit_columns(ws_new)
    ...
    return {
        ...
        "sheet": raw_sheet,
        "created_code_groups": created_code_groups,
    }
...
class VerbatimMapperApp(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.result_panel = QFrame(objectName="ResultPanel")
        result_lay = QVBoxLayout(self.result_panel)
        result_lay.setContentsMargins(0, 0, 0, 0)
        self.result_text = QLabel("พร้อมใช้งาน")
        self.result_text.setObjectName("ResultText")
        self.result_text.setWordWrap(True)
        self.result_text.setAlignment(Qt.AlignmentFlag.AlignHCenter | Qt.AlignmentFlag.AlignVCenter)
        self.result_text.setMinimumHeight(120)
        result_lay.addWidget(self.result_text)
        lay.addWidget(self.result_panel)
    ...
    def info(self, title: str, text: str):
        self._set_result_message(title, text, is_error=False)

    def err(self, title: str, text: str):
        self._set_result_message(title, text, is_error=True)
        self._show_message("error", title, text)

    def _set_result_message(self, title: str, text: str, is_error: bool = False):
        prefix = f"{title}\n" if txt(title) != "" else ""
        shown = f"{prefix}{text}".strip()
        self.result_text.setText(shown if shown else "-")
        if is_error:
            self.result_text.setStyleSheet("color:#fecaca; padding: 10px 12px;")
        else:
            self.result_text.setStyleSheet("")

    def _show_message(self, kind: str, title: str, text: str):
        msg = QMessageBox(self)
        # Use no icon so the text block can be truly centered in the dialog.
        msg.setIcon(QMessageBox.Icon.NoIcon)
        msg.setWindowTitle(title)
        centered_html = f"<div style='text-align:center;'>{escape(text).replace(chr(10), '<br>')}</div>"
        msg.setTextFormat(Qt.TextFormat.RichText)
        msg.setText(centered_html)
    ...
    def _show_completed_popup(self, partial_fail: bool = False):
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Icon.Information)
        msg.setWindowTitle("Completed")
        popup_text = "Completed" if not partial_fail else "Completed\nCheck Summary"
    ...
    def _set_busy(self, busy: bool, text: str = "Processing..."):
        if busy:
            ...
            self.open_output_btn.setEnabled(False)
            self._set_result_message("Processing", "กำลังประมวลผลข้อมูล...")
        ...
        for w in self._controls_to_disable:
            w.setEnabled(True)
        self.open_output_btn.setEnabled(self.last_output_path != "" and os.path.exists(self.last_output_path))
    ...
    def run(self):
        ...
        show_completed_popup = False
        popup_partial_fail = False
        try:
            ...
            elapsed_secs = (self._elapsed.elapsed() / 1000.0) if self._elapsed.isValid() else 0.0
            ...
            text_ok_items = sum(
                1
                for d in details
                if d.get("status") == "SUCCESS" and int(d.get("touched_verbatim", 0)) > 0
            )
            text_ok_rows = sum(int(d.get("touched_verbatim", 0)) for d in details)

            created_groups = res.get("created_code_groups", [])
            if created_groups:
                code_items = [
                    f"{g.get('base', '')} (+{int(g.get('new_cols', 0))} col)"
                    for g in created_groups
                    if txt(g.get("base")) != "" and int(g.get("new_cols", 0)) > 0
                ]
                if not code_items:
                    code_added_summary = "ไม่มี"
                else:
                    show_limit = 6
                    shown = code_items[:show_limit]
                    remain = len(code_items) - len(shown)
                    code_added_summary = ", ".join(shown)
                    if remain > 0:
                        code_added_summary += f" และอีก {remain} ตัว"
            else:
                code_added_summary = "ไม่มี"

            summary_text = (
                f"ใช้เวลา: {elapsed_mins:.2f} นาที\n"
                f"แปะ Text สำเร็จ: {text_ok_items}/{len(var_configs)} ข้อ ({text_ok_rows} แถว)\n"
                f"ตัวแปลโค้ดใหม่ที่เพิ่ม: {code_added_summary}\n"
                f"Merge items used: {merge_count}/{len(var_configs)}"
            )
            if has_fail:
                self.info("Completed", f"Completed (some items failed - check Summary sheet)\n{summary_text}")
            else:
                self.info("Completed", f"Completed\n{summary_text}")
            self.status.setText(f"Completed ({elapsed_mins:.2f} min)")
            self.progress.setRange(0, 1)
            self.progress.setValue(1)
            self.last_output_path = out
            show_completed_popup = True
            popup_partial_fail = has_fail
        except Exception as e:
        ...
        if show_completed_popup:
            self._show_completed_popup(partial_fail=popup_partial_fail)

    def open_output_folder(self):
        target = txt(self.last_output_path)
        if target == "" or not os.path.exists(target):
            self.err("Error", "Output file not found")
            return
        folder = os.path.dirname(target)
        if folder == "" or not os.path.isdir(folder):
            self.err("Error", "Output folder not found")
            return
        try:
            os.startfile(folder)
        except Exception as e:
            self.err("Error", f"Cannot open output folder: {e}")
    ...
    def clear_form(self):
        ...
        self._set_result_message("Ready", "ล้างข้อมูลแล้ว เลือกไฟล์ใหม่ได้เลย")
    ...
    def load_settings_items(self):
        ...
        if not p:
            return
        try:
            items, warnings = load_items_from_settings_file(p)
